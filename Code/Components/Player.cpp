...
void CPlayerComponent::InitializeLocalPlayer ()
{
  ...
  m_pInputComponent->RegisterAction ( "player", "runforward", [this]( int activationMode, float value ) { HandleInputFlagChange ( EInputFlag::RunForward, (EActionActivationMode) activationMode ); } );
  m_pInputComponent->BindAction ( "player", "runforward", eAID_KeyboardMouse, EKeyId::eKI_LShift );
                                        ...
                                        if (activationMode == eAAM_OnPress)
                                        {
                                          if (ICharacterInstance* pCharacter = m_pAnimationComponent->GetCharacter ())
                                          {
                                            ...
                                            if (pBarrelOutAttachment != nullptr)
                                            {
                                              ...
                                              spawnParams.vPosition = bulletOrigin.t + Vec3 ( -0.9f );
...
void CPlayerComponent::UpdateMovementRequest ( float frameTime )
{
  ...
  if (m_inputFlags & EInputFlag::MoveForward)
  {
    ...
    {
      velocity.y += moveSpeed * 3 * frameTime;
    }
  }
  {
  }
  ...
  if ((m_inputFlags & EInputFlag::MoveForward) && m_runningForward)
  {
    m_runningForward = true;

  } else
  {
    m_runningForward = false;
  }
...
void CPlayerComponent::UpdateAnimation ( float frameTime )
{
  ...
  FragmentID desiredFragmentId = m_pCharacterController->IsWalking () ? m_walkFragmentId : m_idleFragmentId;
  if (m_runningForward)
  {
    desiredFragmentId = m_runFragmentId;
  }
...
void CPlayerComponent::HandleInputFlagChange ( const CEnumFlags<EInputFlag> flags, const CEnumFlags<EActionActivationMode> activationMode, const EInputFlagType type )
{
  switch (type)
  {
    ...
    {
      if (flags & EInputFlag::RunForward)
      {
        m_runningForward = true;
      } else
      {
